...
$admin=isset($_POST["admin"])? limpiarCadena($_POST["admin"]):"";
...
switch ($_GET["op"]){
	...
	case 'guardaryeditar':
		...
		if (empty($idusuario)){
			$rspta=$usuarios->insertar($nombre,$direccion,$telefono,$login,$clavehash,$idzona,$admin,$_POST['permiso']);
			echo $rspta ? "Usuario registrado" : "Usuario no se pudo registrar";
		}
		else {
			$rspta=$usuarios->editar($idusuario,$nombre,$direccion,$telefono,$login,$clavehash,$idzona,$admin,$_POST['permiso']);
...
case 'listar':
	...
	break;
		...
		if (isset($fetch))
	    {
			...
			$_SESSION['zona']=$fetch->idzona;
			$_SESSION['admin']=$fetch->is_admin;
			...
			in_array(8,$valores)?$_SESSION['Zonas']=1:$_SESSION['Zonas']=0;
